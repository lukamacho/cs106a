...
public class FacePamphlet extends Program implements FacePamphletConstants {
	...
	private JTextField siblingText;
	private JButton sibling;
	public void init() {
		nameLabel();
		nametext();
		addButton();
	...
	private void siblingButton() {
		sibling = new JButton("Sibling");
		add(sibling, WEST);
	}

	private void siblingText() {
		siblingText = new JTextField(TEXT_FIELD_SIZE);
		add(siblingText, WEST);
		
	}
	/*
	 * This code defines the sex of the profile by combobox.
	 */
	private String genderDefinition() {
		String name=(String) gender.getSelectedItem();
		return name;
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == siblingText || e.getSource() == sibling) {
			siblingAddition();
		}
	}

	/*
	 * This method adds sibling to this profile of siblings.
	 */
	private void siblingAddition() {
		canvas.clearAll();
		profile = data.getProfile(nametext.getText());
		if (data.containsProfile(nametext.getText())) {
			if (data.containsProfile(siblingText.getText())) {
				profile.addSibling(siblingText.getText());
				data.getProfile(siblingText.getText()).addSibling(profile.getName());
				canvas.displayProfile(profile);
				canvas.showMessage("<" + sibling.getText() + "> added as a sibling");
			} else {
				canvas.showMessage("<" + sibling.getText() + "> doesn't exist");
			}
		} else {
	...
	private void deleteProfile() {
		...
		if (data.containsProfile(nametext.getText())) {
			...
			profile.removeSibling(nametext.getText());
	...
	private void profileAdd() {
		...
		profile.setgender(genderDefinition());
		canvas.displayProfile(profile);
		if (data.containsProfile(nametext.getText())) {
