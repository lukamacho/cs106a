...
public class NameSurferGraph extends GCanvas implements NameSurferConstants, ComponentListener {
	...
	/*
	 * This program paints table on the screen.
	 */
	public void tableFormation() {
		if (k % 2 == 1) {
			removeAll();
			paintTableHorizontalLines();
			paintTableVerticalLines();
			writeTableNames();
			writeTableNamesRanks();
			writeTableDecades();
			paintTableUpperLine();
			update();
		}
	...
	/*
	 * This program writes tables upper line.
	 */
	private void paintTableUpperLine() {
		double x1 = 0;
		double x2 = getWidth();
	...
	/*
	 * This program write decades in the table.
	 */
	private void writeTableDecades() {
		for (int i = 0; i < NDECADES; i++) {
			int dec = START_DECADE + 10 * i;
			String str = String.valueOf(dec);
			double x1 = getWidth() / 12 * (i + 1) + 2;
			double y1 = 19;
	...
	/*
	 * This program writes names appropriate ranks in the table.
	 */
	private void writeTableNamesRanks() {
		for (int i = 0; i < database.size(); i++) {
			NameSurferEntry obj = database.get(i);
			for (int j = 0; j < 11; j++) {
				double x1 = getWidth() / 12 * (j + 1) + 3;
				double y1 = i * getHeight() / database.size() + 20 + GRAPH_MARGIN_SIZE;
				GLabel name = new GLabel(" " + obj.getRank(j));
				int fontSize = (int) getWidth() * 15 / APPLICATION_WIDTH;
	...
	/*
	 * This prorgam writes names in the table.
	 */
	private void writeTableNames() {
		for (int i = 0; i < database.size(); i++) {
			NameSurferEntry obj = database.get(i);
			double x1 = 0;
			double y1 = i * getHeight() / database.size() + 20 + GRAPH_MARGIN_SIZE;
			GLabel tableNames = new GLabel(obj.getName());
			add(tableNames, x1, y1);
		}
	}

	/*
	 * This program paint vertical lines which makes up table.
	 */
	private void paintTableVerticalLines() {
		for (int i = 0; i < database.size(); i++) {
			double x1 = 0;
			double x2 = getWidth();
			double y1 = (i + 1) * getHeight() / database.size();
			GLine verticalLines = new GLine(x1, y1, x2, y1);
			add(verticalLines);
		}
	}

	/*
	 * This program paint horizontal lines which makes up table.
	 */
	private void paintTableHorizontalLines() {
		for (int i = 0; i < 12; i++) {
			double y1 = 0;
			double x2 = getWidth() / 12 + i * getWidth() / 12;
			double y2 = getHeight();
			GLine tableLine = new GLine(x2, y1, x2, y2);
			add(tableLine);
		}
	}

	/*
	 * This code deletes the name which you entered in the Name from the arraylist.
	 */
